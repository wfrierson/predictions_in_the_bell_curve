...
6. For each cutoff value, calculate: 
    ...
    * Bias-corrected accelerated (BCa) 95% confidence intervals[^13] for training data
...
## Reproducing This Project{#reproduceProject}

For readers that would like to reproduce this project: 

* Clone the [git repo](https://github.com/wfrierson/predictions_in_the_bell_curve)

* Install the relevant packages listed in [000_Misc.R](https://github.com/wfrierson/predictions_in_the_bell_curve/blob/master/200%20Code/000_Misc.R). If there are versioning issues, please see the section listing my [R session info](#sessionInfo).

* Execute the following line of code from the project directory, which will launch the recursive batch runs for each of the 24 TBC models:

> source('200 Code/TBC_Bootstrap.R', encoding = 'UTF-8', echo=FALSE)

**Notes**: 

1. When 10,000 bootstrap iterations are used on all 24 models in the prior script (with parallel computing), the entire recursive batch process:

    - Produces 193 files in the "100 Data" folder, 6.56 MBs in total

    - Takes 4 - 5 hours

2. Review the initial parameters for the batch run procedure on [lines 55 - 57](https://github.com/wfrierson/predictions_in_the_bell_curve/blob/master/200%20Code/TBC_Bootstrap.R#L55) to make sure they're reasonable for your machine, namely, whether to enable parallel processing and how many CPUs to use.
...
|     Both zAFQT89 and zSES tend to have a monotonically decreasing relationship with the poverty target. The plot with zAFQT89 is the least noisy of the 3 and that of zAge is the most noisy. The modeled probability lines show that zAFQT89 and zSES influence the poverty target, as seen by their:

* Curves significantly varying across the x-axis, and

* 95% confidence intervals not being large enough to allow the possibility of a flat line. 

On the other hand, zAge has extremely wide confidence intervals that could include a flat line. (This is expected, though, since zAge was included as a control and not a parameter that would impact the target.)
...
|     The solid blue line represents the MCC values calculated from the original data (hence its large noise). The dashed blue line represents the mean bootstrapped MCC values. The light blue ribbon represents the 95% confidence intervals for MCC values. The bar chart above the main plot represents the proportion of non-NULL MCC values calculated for that cutoff value. (MCC is undefined when its denominator is 0, which can happen when there are no true positive classifications, e.g.). Lastly, the black vertical dashed line identifies the optimal cutoff probability within that dataset. The adjacent table shows the optimal cutoff probabilities and MCC values (split by the mean bootstrapped value, apparent value from the original data, and optimism-corrected value), including their 95% confidence intervals.

<br/>

__Note__: *The confidence interval for the training data set is noiser than that from the holdout data, because the training data uses bias-corrected accelerated (BCa) confidence intervals whereas the holdout data uses bootstrapped percentile confidence intervals. BCa intervals (roughly) expand upon the noise observed in the apparent intervals.*
...
|     Overall, the average classifications poorly reflect the observed values for both datasets, which is consistent with their optimal MCC values below 35%. If the model performed very well, the blue and grey confidence intervals would have larger overlapping regions across the zAFQT89 buckets with larger weights. For these two plots, the confidence intervals rapidly deviate from each other for zAFQT89 values below -1. For regions where the weights are largest (e.g., zAFQT89 = 0), the confidence intervals do not even intersect.
...
ggplot(
  ...
  xlab('Optimal MCC Value (Training Data, optimism-corrected)') + 
  ylab('Optimal MCC Value (Holdout Data, optimism-corrected)') + 
...
What does the [plot comparing optimal MCC values](#optimalMCC) mean? Using the [guidelines for interpreting MCC values](#mcc) listed above, nearly all TBC models have a low correlation (i.e., MCC < 50%) between their observed binary target and the bootstrapped classification when evaluated on either training or holdout data. The classification plots tell a similar story: the confidence intervals for the optimal classifications tend to not follow the general trend of the observed data. 

|     The only TBC models with optimal MCC values near 50% are those with binary targets related to education: receiving a GED, receiving a Bachelor's, or dropping out of school. Although I am not a social psychologist, these results seem plausible, i.e., having lower IQ has a stronger association with having academic issues. Again using the above guidelines for interpreting correlations, the optimal MCC values for these models are "moderate." Even though the model for "Get Bachelors" had the largest MCC value of `r outputs[label == 'get_bachelors', paste0(floor(mcc.optimism.corrected.training * 1e4)/1e2, '%')]`, its optimal classifications did not materially follow the observed data. While these three models are relatively more predictive than the other TBC models, they alone do not warrant the authors' recommendations.  

<br/>

*Combined, these properties suggest that TBC's probability models are not materially predictive and so they inadequately support the authors' public policy recommendations.*
...
## Table of Optimal MCC Values {#tableOptimalMCC .tabset .tabset-fade}
...
One way to test whether the results of this analysis are reasonable is to reproduce part of it using the `boot` package, which facilitates bootstrapping and can calculate BCa intervals. In the script used to store the project's results (i.e., [TBC_Bootstrap.R](https://github.com/wfrierson/predictions_in_the_bell_curve/blob/master/200%20Code/TBC_Bootstrap.R)), the `boot::boot` function performs the following steps:
...
3. I'm aware bootstrap percentile confidence intervals may have issues. However, I've included them for holdout data as a soft comparison with the results from the training data. The holdout already has the issue of being much thinner relative to the training data for each model.
...
5. I chose 10,000 bootstrap iterations *because* it seemed excessive. Prelim work with the poverty model (not included here) suggested that even 5,000 iterations was adequate. I wanted enough iterations that a reader wouldn't discredit the conclusions for having too few bootstrap iterations. I guess there could be a section about the convergence of the posted values, but that would take more development time than I have.

6. For completeness, it would be nice to explain BCa intervals, but I think that get's too detailed and distracts from the rest of this report.

7. I chose not to use the `boot` package for the main analysis because I wanted to better understand bootstrapping. Also, I couldn't get the boot function to work in parallel (on Windows 10), which greatly extended the overall runtime!

8. The suggested table for interpreting correlation values is only used as a common reference for myself and the reader. Interpretation is always subjective.

9. It's odd that some of the MCC distributions don't have local optima. E.g., see the plot for Unemployed (Highschool Subset). Inspecting the data and code, I still can't explain it. I suspect this has to do with these data sets having fewer NLSY79 participants.

10. I included bootstrap optimism because I wanted to account for sampling bias, especially for models having thin training data. I also use it to give the results a bit more credibility by estimating and removing (a type of) bias. Looking at the [Table of Optimal MCC Values](#tableOptimalMCC), only a few models had noticeably large optimism estimates, and most of those had thin training data (e.g., highschool or college subsets).

11. The code needs better documentation, especially regarding parameters and returned values.
